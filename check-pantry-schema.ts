import { createClient } from '@supabase/supabase-js';

async function checkSchema() {
  const supabase = createClient(
    'https://hfndfmtxhqvubnfiwzlz.supabase.co',
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhmbmRmbXR4aHF2dWJuZml3emx6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA2Mjk4MDgsImV4cCI6MjA3NjIwNTgwOH0.n0NK_Ov03-UbDQYr5mio3ggYa5XTN-XI1kB6X81N4nA',
    {
      global: {
        headers: {
          Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..f2EZLhKbaRxVq0mqJEEKdw7jXf7AiiXdr92MwJh4IYk'
        }
      }
    }
  );

  console.log('Checking for existing tables...\n');

  const { error: pantryError, count } = await supabase
    .from('pantry_items')
    .select('*', { count: 'exact', head: true });

  if (!pantryError) {
    console.log('✅ pantry_items table exists with', count, 'rows');
  } else {
    console.log('❌ pantry_items table does not exist:', pantryError.message);
  }
}

checkSchema();
